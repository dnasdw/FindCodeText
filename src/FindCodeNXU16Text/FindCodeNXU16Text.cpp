...
int UMain(int argc, UChar* argv[])
{
	...
	if (nFindMethod == 1)
	{
		for (u32 i = 0; i < uUncompressedSize / 8 * 8; i += 8)
		{
			...
			if (uRamOffset == uRamOffset64 && uRamOffset % 2 == 0 && uRamOffset >= pNsoHeader->TextMemoryOffset && uRamOffset < pNsoHeader->DataMemoryOffset + pNsoHeader->DataSize)
			{
				U16String sTxtU16 = reinterpret_cast<Char16_t*>(pUncompressed + uRamOffset);
				wstring sTxt;
				try
				{
					sTxt = U16ToW(sTxtU16);
	...
	else if (nFindMethod == 2)
	{
		...
		for (u32 i = pNsoHeader->TextMemoryOffset; i < pNsoHeader->TextMemoryOffset + (pNsoHeader->TextMemoryOffset + pNsoHeader->TextSize - pNsoHeader->TextMemoryOffset) / 4 * 4; i += 4)
		{
			...
			for (u32 j = i + 4; j < uIns2End; j += 4)
			{
				...
				if (uRamOffset % 2 == 0 && uRamOffset >= pNsoHeader->TextMemoryOffset && uRamOffset < pNsoHeader->DataMemoryOffset + pNsoHeader->DataSize)
				{
					U16String sTxtU16 = reinterpret_cast<Char16_t*>(pUncompressed + uRamOffset);
					wstring sTxt;
					try
